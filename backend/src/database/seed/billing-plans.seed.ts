/**
 * Canonical billing plan/feature seed data and an idempotent seeder.
 * Single source of truth shared by the deploy-time seed and the runtime
 * boot-time auto-repair (DatabaseSeedBootstrapService).
 *
 * `register()` -> startTrialSubscription looks up the `professional` plan;
 * if plans are missing, registration commits the user/org then throws, so
 * this data is auth-critical and boot-repaired alongside RBAC.
 */
...
export const FEATURE_SEEDS: FeatureSeed[] = [
  ...
  {
    key: 'communications',
    name: 'Messages sent/received',
    unit: 'COUNT',
    category: 'Communications',
  },
  {
    key: 'email_accounts',
    name: 'Connected email accounts',
    unit: 'COUNT',
    category: 'Communications',
  },
  {
...
export async function seedBillingPlans(client: PrismaClient): Promise<void> {
  ...
  for (const feature of FEATURE_SEEDS) {
    const record = await client.feature.upsert({
  ...
  for (const plan of PLAN_SEEDS) {
    ...
    if (missingFeatures.length > 0) {
      throw new Error(
        `Plan "${plan.key}" references unknown feature(s): ${missingFeatures.join(', ')}`,
      );
    }

    const record = await client.plan.upsert({
      where: { key: plan.key },
      create: {
        key: plan.key,
        name: plan.name,
    ...
    for (const [featureKey, limit] of Object.entries(plan.limits)) {
      ...
      await client.featureLimit.upsert({
