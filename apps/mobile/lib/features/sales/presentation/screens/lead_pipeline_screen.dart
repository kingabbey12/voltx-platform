...
class LeadPipelineScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final companies = ref.watch(companiesProvider(const SalesPageQuery(limit: 100)));
    ...
    final companyMap = {
      for (final company in companies.valueOrNull?.items ?? const <SalesCompany>[]) company.id: company,
    };
    final contactMap = {
      for (final contact in contacts.valueOrNull?.items ?? const <SalesContact>[]) contact.id: contact,
    };
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              onChanged: (value) => ref.read(leadSearchProvider.notifier).state = value,
            ),
            const SizedBox(height: AppSpacing.md),
            Wrap(
              ...
              children: [
                ...
                for (final value in salesLeadStatuses)
                  FilterChip(
                    ...
                    onSelected: (_) => ref.read(leadStatusFilterProvider.notifier).state = value,
                  ),
              ],
            ),
                  ...
                  for (final lead in page.items) ...[
                    SalesSectionCard(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if (lead.companyId != null || lead.contactId != null)
                            Text(
                              [
                                if (lead.companyId != null && companyMap[lead.companyId] != null)
                                  companyMap[lead.companyId]!.name,
                                if (lead.contactId != null && contactMap[lead.contactId] != null)
                                  contactMap[lead.contactId]!.fullName,
                              ].join(' · '),
                            ),
                          if (lead.qualificationScore != null) ...[
                            const SizedBox(height: AppSpacing.xs),
                            Text('Qualification score: ${lead.qualificationScore}'),
                          ],
