...
export default function AiWorkflowTemplatesPage() {
  ...
  async function handleInstantiate() {
    if (!selected) return;
    try {
      const workflow = await instantiate.mutateAsync({
        key: selected.key,
        name: name || undefined,
      });
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Start from a pre-built template instead of an empty canvas."
      />
                    ...
                    <CardContent className="flex flex-1 flex-col justify-between gap-3 pt-0">
                      <p className="text-xs text-muted-foreground">
                        {template.description ??
                          `${template.definition.steps.length} step${template.definition.steps.length === 1 ? "" : "s"}`}
                      </p>
