...
function definitionToGraph(
  definition: WorkflowDefinition,
  errorsByStepId: Record<string, string>,
) {
...
function BuilderCanvas({ workflowId }: { workflowId: string }) {
  ...
  const graph = useMemo(
    () =>
      definition
        ? definitionToGraph(definition, errorsByStepId)
        : { nodes: [], edges: [] },
  ...
  const updateStep = useCallback(
    (stepId: string, next: WorkflowStepDefinition) => {
      ...
      commit({
        ...definition,
        steps: definition.steps.map((s) => (s.id === stepId ? next : s)),
      });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [definition],
    ...
    commit(
      {
        ...definition,
        steps: definition.steps.map((s) =>
          s.id === node.id ? { ...s, layout: node.position } : s,
        ),
      },
      false,
    );
      ...
      next = {
        ...
        steps: next.steps.map((s) =>
          s.id === edge.target
            ? {
                ...s,
                dependsOn: (s.dependsOn ?? []).filter((d) => d !== edge.source),
              }
            : s,
        ),
      };
  ...
  if (workflowLoading || versionsLoading || !definition)
    return <LoadingScreen />;
  if (!workflow) return null;

  const selectedStep =
    definition.steps.find((s) => s.id === selectedStepId) ?? null;
  const allOtherStepIds = definition.steps
    .map((s) => s.id)
      ...
      <div className="flex items-center justify-between gap-4 border-b border-border px-4 py-2.5">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/ai/workflows/${workflowId}`)}
          >
          ...
          <Badge
            variant={
              workflow.status === "PUBLISHED" ? "success" : "secondary"
            }
          >
            {workflow.status}
          </Badge>
          ...
          {CATEGORY_ORDER.map((category) => (
            <div key={category} className="mb-4">
              ...
              <div className="flex flex-col gap-1.5">
                {STEP_TYPE_CATALOG.filter((s) => s.category === category).map(
                  (spec) => (
                    <div
                      key={spec.type}
                      draggable
                      ...
                      onClick={() =>
                        addStep(spec.type, {
                          x: 40,
                          y: 40 + definition.steps.length * 90,
                        })
                      }
